...
# Este es un cambio hecho por el compañero
...
def plot_series(data, initial_date, proy):
    ...
    if not datos_proyeccion.empty:
        ...
        fig.add_trace(go.Scatter(
            name='Upper Bound',
            x=datos_proyeccion.index,
            y=datos_proyeccion['Upper bound'],
            mode='lines',
        ...
        fig.add_trace(go.Scatter(
            name='Lower Bound',
            x=datos_proyeccion.index,
            y=datos_proyeccion['Lower bound'],
            marker=dict(color="#444"),
            line=dict(width=0),
            mode='lines',
...
def update_output_div(date, hour, proy):
    ...
    if ((date is not None) & (hour is not None) & (proy is not None)):
        initial_date_str = f"{date} {hour:02d}:00"
